...
/*
  ...
  const handleSearch = async () => {
    setSearching(true);
    try {
      const response = await axios.post('http://localhost:3000/result', { title, year });
  ...
  const handleSendFilm = async () => {
    try {
      if (movie.isFavorite){
        await axios.post('http://localhost:3000/remove_favorite', { imdbid: movie.imdbID });
        console.log('Film deleted successfully!');
      } else {
        await axios.post('http://localhost:3000/send_films', { imdbid: movie.imdbID });
